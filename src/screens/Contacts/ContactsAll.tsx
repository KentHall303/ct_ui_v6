...
interface ContactsAllHeaderProps {
  totalRecords: number;
...
const ContactsAllHeader: React.FC<ContactsAllHeaderProps> = ({
  totalRecords,
  selectedCount,
  currentPage,
  totalPages,
  ...
  return (
    <div className="px-3 pt-3">
      <div className="bg-white rounded-3 pt-3 pb-3 px-4 border shadow-sm">
        <h2 className="h4 fw-bold text-dark mb-2">Contacts - All</h2>
        <div className="d-flex align-items-center justify-content-between">
          <p className="text-secondary mb-0" style={{ fontSize: '0.875rem' }}>
            {totalRecords} Records in All Contacts
        ...
        <div className="py-0 mt-3">
          <div className="d-flex align-items-center justify-content-between">
            ...
            <div className="d-flex align-items-center gap-2">
              <Button variant="link" size="sm" className="p-1 text-decoration-none" onClick={() => onPageChange(1)} disabled={currentPage === 1}>
                <ChevronsLeftIcon size={16} />
              </Button>
              <Button variant="link" size="sm" className="p-1 text-decoration-none" onClick={() => onPageChange(currentPage - 1)} disabled={currentPage === 1}>
              ...
              <div className="d-flex gap-1">
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <Button key={pageNum} variant={currentPage === pageNum ? "primary" : "link"} size="sm" className="px-2 py-1" onClick={() => onPageChange(pageNum)}>
                      {pageNum}
                    </Button>
                  );
                })}
              </div>
              <Button variant="link" size="sm" className="p-1 text-decoration-none" onClick={() => onPageChange(currentPage + 1)} disabled={currentPage === totalPages}>
                <ChevronRightIcon size={16} />
              </Button>
              <Button variant="link" size="sm" className="p-1 text-decoration-none" onClick={() => onPageChange(totalPages)} disabled={currentPage === totalPages}>
            ...
            <div className="d-flex align-items-center gap-2">
              ...
              <Button variant="link" size="sm" className="p-1 text-decoration-none" onClick={onOpenFilters} title="Advanced Filters">
                <FilterIcon size={16} />
              </Button>
              <Button variant="link" size="sm" className={`p-1 text-decoration-none ${hasActiveFilters ? 'text-danger' : ''}`} onClick={onClearFilters} title="Clear Filters">
                <XIcon size={16} />
              </Button>
              <Button variant="link" size="sm" className="p-1 text-decoration-none" onClick={onOpenSettings}>
...
export const ContactsAll = (): JSX.Element => {
  ...
  const { columnWidths, isResizing, resizingColumn, handleMouseDown } = useResizableColumns(allColumns, 'contactsAllColumns');
                    ...
                    <TableCell style={{ width: `${columnWidths.name}px` }} role="gridcell">
                      {isEditing ? (
                        <div className="d-flex align-items-center gap-2">
                          <Form.Select
                            size="sm"
                            value={editedValues.contact_type || contact.contact_type || 'Client'}
                            onChange={(e) => updateEditedField('contact_type', e.target.value)}
                            style={{ width: '90px' }}
                          >
                            {contactTypeOptions.map((type) => (
