...
const SeasonalPlans = (): JSX.Element => {
  ...
  const [showInfo, setShowInfo] = React.useState(() => {
    const saved = localStorage.getItem('seasonalPlansInfoVisible');
  ...
  const {
    ...
    resizingColumn,
    handleMouseDown,
  } = useResizableColumns(columns, 'seasonalPlansColumns');
  ...
  const toggleInfo = () => {
    setShowInfo((prev) => {
      const newValue = !prev;
      localStorage.setItem('seasonalPlansInfoVisible', String(newValue));
      return newValue;
    });
  };
  ...
  const loadPlans = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await connectionPlanService.getByPlanType('Seasonal Plans');
      setPlans(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load seasonal plans');
      console.error('Error loading seasonal plans:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDuplicatePlan = async (plan: ConnectionPlan) => {
    try {
      ...
      console.error('Error duplicating seasonal plan:', err);
      alert('Failed to duplicate seasonal plan. Please try again.');
    } finally {
      setDuplicating(null);
    }
  };
  ...
  if (loading) {
    return (
      <div className="d-flex flex-column w-100 h-100 align-items-center justify-content-center">
        ...
        <p className="mt-3 text-muted">Loading seasonal plans...</p>
      </div>
    );
  }
          ...
          <div className="d-flex align-items-start justify-content-between">
            <div className="flex-grow-1" style={{ maxWidth: 'calc(100% - 140px)' }}>
              <div className="d-flex align-items-center gap-2 mb-1">
                <h2 className="h4 fw-bold text-dark mb-0">Seasonal Plans</h2>
                <button
                  onClick={toggleInfo}
              ...
              <Collapse in={showInfo}>
                <div>
                  ...
                  <p className="text-secondary mb-2" style={{ fontSize: '0.9375rem', lineHeight: '1.5' }}>
                    Seasonal Plans are designed for time-based campaigns tied to holidays, quarters,
                    or specific seasons. These plans help businesses stay connected with customers
                    during key periods throughout the year.
                  </p>
                  <a
        ...
        >
          <Table className={`standard-table table-striped mb-0 ${isResizing ? 'resizing' : ''}`}>
            <caption className="visually-hidden">
              Seasonal plans table showing {plans.length} records.
...
export const SeasonalPlansPage: React.FC = (): JSX.Element => {
  return (
    <BodyLayout>
      <SeasonalPlans />
    </BodyLayout>
