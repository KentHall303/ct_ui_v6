...
const RetentionPlans = (): JSX.Element => {
  ...
  const [showInfo, setShowInfo] = React.useState(() => {
    const saved = localStorage.getItem('retentionPlansInfoVisible');
  ...
  const {
    ...
    resizingColumn,
    handleMouseDown,
  } = useResizableColumns(columns, 'retentionPlansColumns');
  ...
  const toggleInfo = () => {
    setShowInfo((prev) => {
      const newValue = !prev;
      localStorage.setItem('retentionPlansInfoVisible', String(newValue));
      return newValue;
    });
  };
  ...
  const loadPlans = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await connectionPlanService.getByPlanType('Retention Plans');
      setPlans(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load retention plans');
      console.error('Error loading retention plans:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDuplicatePlan = async (plan: ConnectionPlan) => {
    try {
      ...
      console.error('Error duplicating retention plan:', err);
      alert('Failed to duplicate retention plan. Please try again.');
    } finally {
      setDuplicating(null);
    }
  };
  ...
  if (loading) {
    return (
      <div className="d-flex flex-column w-100 h-100 align-items-center justify-content-center">
        ...
        <p className="mt-3 text-muted">Loading retention plans...</p>
      </div>
    );
  }
          ...
          <div className="d-flex align-items-start justify-content-between">
            <div className="flex-grow-1" style={{ maxWidth: 'calc(100% - 140px)' }}>
              <div className="d-flex align-items-center gap-2 mb-1">
                <h2 className="h4 fw-bold text-dark mb-0">Retention Plans</h2>
                <button
                  onClick={toggleInfo}
              ...
              <Collapse in={showInfo}>
                <div>
                  ...
                  <p className="text-secondary mb-2" style={{ fontSize: '0.9375rem', lineHeight: '1.5' }}>
                    Retention Plans focus on maintaining relationships with existing customers through
                    regular touchpoints, loyalty programs, and re-engagement campaigns to maximize
        ...
        >
          <Table className={`standard-table table-striped mb-0 ${isResizing ? 'resizing' : ''}`}>
            <caption className="visually-hidden">
              Retention plans table showing {plans.length} records.
                ...
                >
                  ...
                  <TableCell role="gridcell" style={{ width: `${columnWidths.action}px` }}>
                    <div className="d-flex gap-2 justify-content-center">
                      ...
                      <button
                        className="btn btn-link p-0 border rounded-circle d-flex align-items-center justify-content-center"
                        title="Duplicate retention plan"
                        style={{
                          width: '24px',
                          height: '24px',
                          borderColor: '#dee2e6',
...
export const RetentionPlansPage: React.FC = (): JSX.Element => {
  return (
    <BodyLayout>
      <RetentionPlans />
    </BodyLayout>
