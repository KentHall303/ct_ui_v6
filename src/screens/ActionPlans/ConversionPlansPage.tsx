...
const ConversionPlans = (): JSX.Element => {
  ...
  const [showInfo, setShowInfo] = React.useState(() => {
    const saved = localStorage.getItem('conversionPlansInfoVisible');
  ...
  const {
    ...
    resizingColumn,
    handleMouseDown,
  } = useResizableColumns(columns, 'conversionPlansColumns');
  ...
  const toggleInfo = () => {
    setShowInfo((prev) => {
      const newValue = !prev;
      localStorage.setItem('conversionPlansInfoVisible', String(newValue));
      return newValue;
    });
  };
  ...
  const loadPlans = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await connectionPlanService.getByPlanType('Conversion Plans');
      setPlans(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load conversion plans');
      console.error('Error loading conversion plans:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDuplicatePlan = async (plan: ConnectionPlan) => {
    try {
      ...
      console.error('Error duplicating conversion plan:', err);
      alert('Failed to duplicate conversion plan. Please try again.');
    } finally {
      setDuplicating(null);
    }
  };
  ...
  if (loading) {
    return (
      <div className="d-flex flex-column w-100 h-100 align-items-center justify-content-center">
        ...
        <p className="mt-3 text-muted">Loading conversion plans...</p>
      </div>
    );
  }
          ...
          <div className="d-flex align-items-start justify-content-between">
            <div className="flex-grow-1" style={{ maxWidth: 'calc(100% - 140px)' }}>
              <div className="d-flex align-items-center gap-2 mb-1">
                <h2 className="h4 fw-bold text-dark mb-0">Conversion Plans</h2>
                <button
                  onClick={toggleInfo}
              ...
              <Collapse in={showInfo}>
                <div>
                  ...
                  <p className="text-secondary mb-2" style={{ fontSize: '0.9375rem', lineHeight: '1.5' }}>
                    Conversion Plans are designed to convert leads into customers through strategic,
                    multi-touch outreach sequences. These plans focus on nurturing prospects through
                    the sales funnel with targeted communications and follow-ups.
                  </p>
                  <a
        ...
        >
          <Table className={`standard-table table-striped mb-0 ${isResizing ? 'resizing' : ''}`}>
            <caption className="visually-hidden">
              Conversion plans table showing {plans.length} records.
                ...
                >
                  ...
                  <TableCell role="gridcell" style={{ width: `${columnWidths.action}px` }}>
                    <div className="d-flex gap-2 justify-content-center">
                      ...
                      <button
                        className="btn btn-link p-0 border rounded-circle d-flex align-items-center justify-content-center"
                        title="Duplicate conversion plan"
                        style={{
                          width: '24px',
                          height: '24px',
                          borderColor: '#dee2e6',
...
export const ConversionPlansPage: React.FC = (): JSX.Element => {
  return (
    <BodyLayout>
      <ConversionPlans />
    </BodyLayout>
