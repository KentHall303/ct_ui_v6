...
const NotesLogsTemplates = (): JSX.Element => {
  const [templates, setTemplates] = React.useState<NotesLogsTemplate[]>([]);
  ...
  const loadTemplates = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await notesLogsTemplateService.getAll();
      setTemplates(data);
    } catch (err) {
  ...
  const handleEditTemplate = (template: NotesLogsTemplate) => {
    setEditingTemplate(template);
    setShowAddModal(true);
  };

  const handleDuplicateTemplate = async (template: NotesLogsTemplate) => {
    try {
      setDuplicating(template.id);
      await notesLogsTemplateService.duplicate(template.id);
      await loadTemplates();
    } catch (err) {
  ...
  const sortedTemplates = React.useMemo(() => {
    ...
    return [...templates].sort((a, b) => {
      let aVal = a[sortConfig.key as keyof NotesLogsTemplate];
      let bVal = b[sortConfig.key as keyof NotesLogsTemplate];
      ...
      <div className="px-3 pt-3 flex-shrink-0">
        <div className="bg-white rounded-3 pt-3 pb-3 px-4 border shadow-sm">
          <div className="d-flex align-items-center justify-content-between">
            <h2 className="h4 fw-bold text-dark mb-0">Notes/Logs Templates</h2>
            <Button
              variant="success"
              size="sm"
...
export const NotesLogsTemplatesPage: React.FC = (): JSX.Element => {
  return (
    <BodyLayout>
      <NotesLogsTemplates />
    </BodyLayout>
