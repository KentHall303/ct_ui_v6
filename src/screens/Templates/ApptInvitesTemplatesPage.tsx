...
const ApptInviteTemplates = (): JSX.Element => {
  const [templates, setTemplates] = React.useState<ApptInviteTemplate[]>([]);
  ...
  const loadTemplates = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await apptInviteTemplateService.getAll();
      setTemplates(data);
    } catch (err) {
  ...
  const handleEditTemplate = (template: ApptInviteTemplate) => {
    setEditingTemplate(template);
    setShowAddModal(true);
  };

  const handleDuplicateTemplate = async (template: ApptInviteTemplate) => {
    try {
      setDuplicating(template.id);
      await apptInviteTemplateService.duplicate(template.id);
      await loadTemplates();
    } catch (err) {
  ...
  const sortedTemplates = React.useMemo(() => {
    ...
    return [...templates].sort((a, b) => {
      let aVal = a[sortConfig.key as keyof ApptInviteTemplate];
      let bVal = b[sortConfig.key as keyof ApptInviteTemplate];
...
export const ApptInvitesTemplatesPage: React.FC = (): JSX.Element => {
  return (
    <BodyLayout>
      <ApptInviteTemplates />
    </BodyLayout>
