...
const TaskTemplates = (): JSX.Element => {
  ...
  const columns: ColumnConfig[] = [
    { id: 'title', label: 'Title', defaultWidth: 450, minWidth: 200 },
    { id: 'assignee_type', label: 'Assignee', defaultWidth: 200, minWidth: 120 },
    { id: 'priority', label: 'Priority', defaultWidth: 150, minWidth: 100 },
  ...
  const {
    ...
    resizingColumn,
    handleMouseDown,
  } = useResizableColumns(columns, 'taskTemplatesColumns');
  ...
  const handleDuplicateTemplate = async (template: TaskTemplate) => {
    try {
      ...
      await taskTemplateService.duplicate(template.id);
      await loadTemplates();
    } catch (err) {
      ...
      <div className="px-3 pt-3 flex-shrink-0">
        <div className="bg-white rounded-3 pt-3 pb-3 px-4 border shadow-sm">
          <div className="d-flex align-items-center justify-content-between">
            <h2 className="h4 fw-bold text-dark mb-0">Task Templates</h2>
            <Button
              variant="success"
              size="sm"
        ...
        >
          <Table className={`standard-table table-striped mb-0 ${isResizing ? 'resizing' : ''}`}>
            <caption className="visually-hidden">
              Task templates table showing {templates.length} records.
            ...
            <TableBody>
              {sortedTemplates.map((template, index) => (
                ...
                >
                  <TableCell role="gridcell" style={{ width: `${columnWidths.title}px` }}>
                    <div
                      className="text-dark"
                      style={{
                        overflow: 'hidden',
                  ...
                  <TableCell role="gridcell" style={{ width: `${columnWidths.assignee_type}px` }}>
                    <div className="text-dark" style={{ fontSize: '0.8125rem' }}>
                      {getAssigneeLabel(template.assignee_type)}
                    </div>
                  </TableCell>

                  <TableCell role="gridcell" style={{ width: `${columnWidths.priority}px` }}>
                    <div className="text-dark" style={{ fontSize: '0.8125rem' }}>
                      {template.priority || '-'}
                    </div>
                  </TableCell>
...
export const TaskTemplatesPage: React.FC = (): JSX.Element => {
  return (
    <BodyLayout>
      <TaskTemplates />
    </BodyLayout>
