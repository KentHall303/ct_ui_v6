/*
  # Create Pipeline Tables for Sales Management

  ## New Tables
  
  ### `sales_cycles`
  - `id` (uuid, primary key) - Unique identifier for each sales cycle stage
  - `name` (text) - Name of the sales cycle stage (e.g., "Lead", "Qualified", "Proposal")
  - `order_position` (integer) - Order position for display in pipeline
  ...
  - `sales_cycle_id` (uuid, foreign key) - Reference to sales_cycles table
  - `estimated_value` (numeric) - Estimated value of the opportunity
  - `priority` (text) - Priority level: 'high', 'medium', or 'low'
  - `lead_source` (text, nullable) - Source of the lead
  - `contact_type` (text) - Type of contact (e.g., "Client", "Prospect")
  - `created_at` (timestamptz) - Timestamp of creation
  - `updated_at` (timestamptz) - Timestamp of last update

  ## Security
  - Enable RLS on both tables
  - Add policies for authenticated users to manage their pipeline data
